...
func main() {
	...
	for len(opMap) < 16 {
		fmt.Printf("%v\n", opMap)
		for op, canBeAt := range opStat {
			if len(canBeAt) == 1 {
				opMap[op] = firstKey(canBeAt)
			}
		}

		for _, canBeAt := range opStat {
			for _, known := range opMap {
				delete(canBeAt, known)
			}
		}
	}
	fmt.Printf("%v\n", opMap)
	...
	for _, l := range program {
		registers = ops[opMap[l.op]](l.A, l.B, l.C, registers)
	}
