...
func NewCave(lines [][]rune) *Cave {
	...
	return &Cave{caveMap, lines, m, dudes, len(lines)*len(lines[0])}
}
...
func (cave *Cave) Tick() bool{
	...
	for _, d := range cave.dudes {
        ...
        fmt.Printf("%d %c %d,%d starts ", d.id, d.fraction, d.pos.x, d.pos.y)
		enemyAround := false
...
func main() {
    ...
    fmt.Printf("\n\nrounds: %d, hp: %d; checksum: %d\n\n", rounds, remainingHp, rounds * remainingHp)
}
